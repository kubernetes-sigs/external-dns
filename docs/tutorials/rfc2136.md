...
explicitly enabled via the `--rfc2136-axfr` flag. This is necessary for
ExternalDNS to list all existing DNS records and determine which ones should be
lifecycled.

Without `--rfc2136-axfr`, ExternalDNS cannot list records and will act as
if the policy was set to `create-only`.

### Unsigned zone transfers (split-server deployments)

By default, when TSIG is configured, ExternalDNS attaches the TSIG key to **both**
dynamic updates (UPDATE) and zone transfers (AXFR). Some deployments serve AXFR
from a different endpoint than the TSIG-gated UPDATE endpoint, and that AXFR
endpoint does not require (or support) TSIG. This is common in split-role setups
where reads and writes are handled by separate servers — for example a hidden
primary that accepts signed updates while zone transfers are served unsigned from
secondaries.

!!! warning
    With `--rfc2136-axfr-insecure`, zone transfer responses are unauthenticated.
    Only use this on a trusted network path, or combine it with `--rfc2136-use-tls`.

The `--rfc2136-axfr-insecure` flag skips TSIG on AXFR/zone-transfer requests while
**UPDATE continues to use TSIG**:
...
This is the only configuration that yields HMAC-TSIG on UPDATE together with an
unsigned AXFR without falling back to GSS-TSIG/Kerberos:

| `--rfc2136-insecure` | `--rfc2136-gss-tsig` | `--rfc2136-axfr-insecure` | UPDATE auth | AXFR auth |
|----------------------|----------------------|---------------------------|-------------|-----------|
| no                   | no                   | no                        | HMAC TSIG   | HMAC TSIG |
| no                   | no                   | **yes**                   | HMAC TSIG   | **none**  |
| yes                  | no                   | any                       | none        | none      |
| no                   | yes                  | any                       | GSS-TSIG    | none      |

Notes:

- The flag only affects AXFR. It is a no-op under `--rfc2136-insecure` (which
  already disables all TSIG) or `--rfc2136-gss-tsig` (under which AXFR is already
  unsigned).
- It requires `--rfc2136-axfr`. Setting it without zone transfers enabled is
  rejected at startup rather than silently ignored. The deprecated
  `--rfc2136-tsig-axfr` also satisfies this, since it is folded into
  `--rfc2136-axfr` before validation.
...
compare equal to the same records read back over AXFR.

Two caveats:

- Reading existing TLSA records needs `--rfc2136-axfr`, like every other record type this provider manages. Without it
  the plan can only create records.
- DANE requires DNSSEC on the zone. Sign it and publish the DS record at your registrar; ExternalDNS does not check
  this.
...
...
        ...
        - --rfc2136-axfr # needed to enable zone transfers, which is required for deletion of records.
...
```
