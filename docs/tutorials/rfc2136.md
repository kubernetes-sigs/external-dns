...
if the policy was set to `upsert-only`. No warning will be provided, and the
`external_dns_controller_deletions_skipped_by_policy` metric will remain at
zero because nothing is ever classified as "to be deleted" — do not rely on
that metric to detect this misconfiguration. To catch it, watch the registry
record count (`external_dns_registry_endpoints_total`): a deployment reconciling
a non-empty source where the registry count stays at zero is the telltale
signature of AXFR being disabled.
