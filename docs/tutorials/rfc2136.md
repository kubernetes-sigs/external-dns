...
> **Note:** The `--rfc2136-create-ptr` flag has been removed. Use the generic `--create-ptr` flag instead,
> which works with any provider — not just RFC2136.

ExternalDNS can automatically create PTR records for your A/AAAA records.
For RFC2136 you also need to add the reverse zone to `--rfc2136-zone`:
