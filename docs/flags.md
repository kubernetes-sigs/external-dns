...
| `--cloudflare-region-key=CLOUDFLARE-REGION-KEY` | When using the Cloudflare provider, specify the region (default: earth). Common values: `earth` (all data centers), `eu`, `us`, `ap`, `fedramp`, `in`, `ca`, `jp`, `kr`, `br`, `za`, `ae`, `global` (alias for earth). See the [Cloudflare Regional Services documentation](https://developers.cloudflare.com/data-localization/regional-services/get-started/) for the full list and details. |
