...
| skipped_records_label_overflow_per_sync | Gauge       | registry         | record_type, domain, overflow_in            | Number of records skipped per sync because a DNS label exceeds RFC 1035's 63-char limit, by record type, apex domain, and whether the overflow is in the source name or the projected TXT registry name. |
