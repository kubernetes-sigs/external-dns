...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.BoolVar("enable-legacy-annotation-prefix", "Also read annotations using the pre-v0.22.0 prefix external-dns.alpha.kubernetes.io/ while migrating them to --annotation-prefix, which takes precedence when both are present (default: disabled)", false, &cfg.EnableLegacyAnnotationPrefix)
