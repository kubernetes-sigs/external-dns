...
type Config struct {
	...
	ResolveGatewayLoadBalancerHostname            bool
	RFC2136Host                                   []string
	RFC2136Port                                   int
	RFC2136Zone                                   []string
...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.BoolVar("resolve-gateway-load-balancer-hostname", "Resolve the hostname of LoadBalancer addresses in Gateway status to IP addresses in order to create DNS A/AAAA records instead of CNAMEs", false, &cfg.ResolveGatewayLoadBalancerHostname)
