...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	managedRecordTypesHelp := fmt.Sprintf("Record types to manage; specify multiple times to include many; (default: %s) (supported records: A, AAAA, CNAME, NS, SRV, TXT, TLSA)", strings.Join(defaultConfig.ManagedDNSRecordTypes, ","))
