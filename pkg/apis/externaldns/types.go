...
var defaultConfig = &Config{
	...
	UnstructuredFQDNResources:      []string{},
	UnstructuredFQDNTargetTemplate: "",
}
...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	// Flags related to unstructured-fqdn source
	b.StringsVar("unstructured-fqdn-resource", "When using the unstructured-fqdn source, specify resources in resource.version.group format (e.g., virtualmachineinstances.v1.kubevirt.io); specify multiple times for multiple resources", nil, &cfg.UnstructuredFQDNResources)
	b.StringVar("fqdn-target-template", "When using the unstructured source, specify the target FQDN template for DNS records", defaultConfig.UnstructuredFQDNTargetTemplate, &cfg.UnstructuredFQDNTargetTemplate)
