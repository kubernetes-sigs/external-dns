...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("infoblox-wapi-port-ro", "When using the Infoblox provider, specify the WAPI port (default: 443)").Default(strconv.Itoa(defaultConfig.InfobloxWapiPortRO)).IntVar(&cfg.InfobloxWapiPortRO)
	app.Flag("infoblox-wapi-username-ro", "When using the Infoblox provider, specify the WAPI username (default: admin)").Default(defaultConfig.InfobloxWapiUsernameRO).StringVar(&cfg.InfobloxWapiUsernameRO)
	app.Flag("infoblox-wapi-password-ro", "When using the Infoblox provider, specify the WAPI password (required when --provider=infoblox)").Default(defaultConfig.InfobloxWapiPasswordRO).StringVar(&cfg.InfobloxWapiPasswordRO)
	app.Flag("infoblox-wapi-version-ro", "When using the Infoblox provider, specify the WAPI version (default: 2.3.1)").Default(defaultConfig.InfobloxWapiVersionRO).StringVar(&cfg.InfobloxWapiVersionRO)
	app.Flag("infoblox-ssl-verify-ro", "When using the Infoblox provider, specify whether to verify the SSL certificate (default: true, disable with --no-infoblox-ssl-verify)").Default(strconv.FormatBool(defaultConfig.InfobloxSSLVerifyRO)).BoolVar(&cfg.InfobloxSSLVerifyRO)
