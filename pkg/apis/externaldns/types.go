...
type Config struct {
	...
	AdguardHomeTLSInsecureSkipVerify   bool
}
...
var defaultConfig = &Config{
	...
	AdguardHomeUsername:              "",
	AdguardHomePassword:              "",
	AdguardHomeTLSInsecureSkipVerify: false,
}
...
func (cfg *Config) ParseFlags(args []string) error {
	...
	// Flags related to AdGuard Home provider
	app.Flag("adguardhome-server", "When using the AdGuard Home provider, the base URL of the AdGuard Home web server (required when --provider=adguardhome)").Default(defaultConfig.AdguardHomeServer).StringVar(&cfg.AdguardHomeServer)
	app.Flag("adguardhome-username", "When using the AdGuard Home provider, the username to the server if it is protected").Default(defaultConfig.AdguardHomeUsername).StringVar(&cfg.AdguardHomeUsername)
	app.Flag("adguardhome-password", "When using the AdGuard Home provider, the password to the server if it is protected").Default(defaultConfig.AdguardHomePassword).StringVar(&cfg.AdguardHomePassword)
	app.Flag("adguardhome-tls-skip-verify", "When using the AdGuard Home provider, disable verification of any TLS certificates").BoolVar(&cfg.AdguardHomeTLSInsecureSkipVerify)
