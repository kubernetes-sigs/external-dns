...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.BoolVar("rfc2136-insecure-axfr", "When using the RFC2136 provider, do not attach TSIG to AXFR/zone-transfer requests (UPDATE still uses TSIG if --rfc2136-tsig-axfr=true). Useful for split-server PowerDNS deployments where the AXFR server is unsigned but the UPDATE server is TSIG-gated.", defaultConfig.RFC2136InsecureAXFR, &cfg.RFC2136InsecureAXFR)
