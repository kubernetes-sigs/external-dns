...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.StringsVar("default-targets", "Set globally default host/IP that will apply as a target instead of source addresses. Applies when source endpoints have empty targets (e.g. CRD source) or to all endpoints when --force-default-targets is enabled. Specify multiple times for multiple targets (optional)", nil, &cfg.DefaultTargets)
