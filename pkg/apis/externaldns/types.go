...
func App(cfg *Config) *kingpin.Application {
	...
	app.Flag("azure-maxretries-count", "When using the Azure provider, set the number of retries for API calls (0 to disable retries).").Default(strconv.Itoa(defaultConfig.AzureMaxRetriesCount)).IntVar(&cfg.AzureMaxRetriesCount)
