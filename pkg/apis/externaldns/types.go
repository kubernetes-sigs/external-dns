...
type Config struct {
	...
	MikrotikServer                     string
	MikrotikUsername                   string `secure:"yes"`
	MikrotikPassword                   string `secure:"yes"`
	MikrotikTLSInsecureSkipVerify      bool
	MikrotikMinimumTTL                 int64
}
...
var defaultConfig = &Config{
	...
	MikrotikUsername:              "",
	MikrotikPassword:              "",
	MikrotikTLSInsecureSkipVerify: false,
	MikrotikMinimumTTL:            60,
}
...
func (cfg *Config) ParseFlags(args []string) error {
	...
	// Flags related to Mikrotik provider
	app.Flag("mikrotik-router", "When using the Mikrotik provider, the base URL of the Mikrotik router (required when --provider=mikrotik)").Default(defaultConfig.MikrotikServer).StringVar(&cfg.MikrotikServer)
	app.Flag("mikrotik-username", "When using the Mikrotik provider, the username to connect as").Default(defaultConfig.MikrotikUsername).StringVar(&cfg.MikrotikUsername)
	app.Flag("mikrotik-password", "When using the Mikrotik provider, the user's password").Default(defaultConfig.MikrotikPassword).StringVar(&cfg.MikrotikPassword)
	app.Flag("mikrotik-tls-skip-verify", "When using the Mikrotik provider, disable verification of any TLS certificates").BoolVar(&cfg.MikrotikTLSInsecureSkipVerify)
	app.Flag("mikrotik-minimum-ttl", "When using the Mikrotik provider, this is the minimum TTL value in seconds (default is 60)").Int64Var(&cfg.MikrotikMinimumTTL)
