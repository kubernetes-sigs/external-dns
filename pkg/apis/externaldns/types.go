...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.StringsVar("namespaces", "Limit resources queried for endpoints to specific namespaces; specify multiple times for multiple namespaces", nil, &cfg.Namespaces)
	b.StringVar("namespace-label-selector", "Limit resources queried for endpoints to namespaces matching the label selector (default: all namespaces)", defaultConfig.NamespaceLabelSelector, &cfg.NamespaceLabelSelector)
