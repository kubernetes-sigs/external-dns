...
func bindFlags(b flags.FlagBinder, cfg *Config) {
	...
	b.StringsVar("zone-id-filter", "Filter target zones by hosted zone id; specify multiple times for multiple zones (optional). Google provider: values are matched as a suffix against the zone name and numeric ID (e.g. 'public' matches 'public' and 'prod-public'). Zone-scoped IAM without project-level dns.managedZones.list requires exact zone names or numeric IDs.", []string{""}, &cfg.ZoneIDFilter)
