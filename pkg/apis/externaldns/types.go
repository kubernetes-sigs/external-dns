...
func App(cfg *Config) *kingpin.Application {
	...
	app.Flag("azure-maxretries-count", " When using the Azure provider, set the number of retries for API calls (Value less than 0 to disable retries). (optional)").Default(strconv.Itoa(defaultConfig.AzureMaxRetriesCount)).IntVar(&cfg.AzureMaxRetriesCount)
