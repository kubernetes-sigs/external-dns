...
func (cfg *Config) ParseFlags(args []string) error {
	...
	app.Flag("aws-assume-role-external-id", "When using the AWS provider, include this external id in the request to assume an IAM role, e.g. `aOor3Ko3` (optional)").Default(defaultConfig.AWSAssumeRoleExternalId).StringVar(&cfg.AWSAssumeRoleExternalId)
