...
func TestSetDualstackServiceLabel(t *testing.T) {
	for _, tc := range []struct {
		name          string
		...
		{
			name:          "load balancer without annotation",
			serviceType:   v1.ServiceTypeLoadBalancer,
			wantDualstack: false,
		},
		{
			name:          "load balancer explicit ipv4",
			serviceType:   v1.ServiceTypeLoadBalancer,
			ipAddressType: "ipv4",
			wantDualstack: false,
		},
		{
			name:          "load balancer dualstack",
			serviceType:   v1.ServiceTypeLoadBalancer,
			ipAddressType: NLBDualstackAnnotationValue,
			wantDualstack: true,
		},
		{
			name:          "cluster ip with dualstack annotation is ignored",
		...
		t.Run(tc.name, func(t *testing.T) {
			annotations := map[string]string{}
			if tc.ipAddressType != "" {
				annotations[NLBDualstackAnnotationKey] = tc.ipAddressType
			}
			...
			ep := endpoint.NewEndpoint(
				...
				"load-balancer.elb.us-east-1.amazonaws.com",
			)

			setDualstackServiceLabel(svc, []*endpoint.Endpoint{ep})
