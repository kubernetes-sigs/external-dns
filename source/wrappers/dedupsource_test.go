...
// withDedupLookupIP is a test-only DedupSourceOption that replaces the DNS lookup function.
func withDedupLookupIP(fn func(string) ([]net.IP, error)) DedupSourceOption {
	return func(ds *dedupSource) {
		ds.lookupIP = fn
	}
}

func TestDedupSourceResolveGlobalFlag(t *testing.T) {
	tests := []struct {
		title     string
	...
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			...
			src := NewDedupSource(ms, WithDedupResolveLoadBalancerHostname(true), withDedupLookupIP(tt.lookupIP))
...
func TestDedupSourceResolvePerAnnotation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.title, func(t *testing.T) {
			...
			src := NewDedupSource(ms, WithDedupResolveLoadBalancerHostname(tt.globalFlag), withDedupLookupIP(stubLookup))
