...
func TestSetDualstackIngressLabel(t *testing.T) {
	for _, tc := range []struct {
		...
		{
			name:          "default without annotation",
			wantDualstack: false,
		},
		{
			name:          "explicit ipv4",
		...
		{
			name:          "dualstack",
			ipAddressType: ALBDualstackAnnotationValue,
			wantDualstack: true,
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			annotations := map[string]string{}
			if tc.ipAddressType != "" {
				annotations[ALBDualstackAnnotationKey] = tc.ipAddressType
			}
			...
			setDualstackIngressLabel(ing, []*endpoint.Endpoint{ep})
