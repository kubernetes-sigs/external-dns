	...
	for _, s := range ms.children {
		...
		if len(ms.defaultTargets) > 0 {
			for i := range endpoints {
				eps := endpointsForHostname(endpoints[i].DNSName, endpoint.NewTargets(ms.defaultTargets...), endpoints[i].RecordTTL, endpoints[i].ProviderSpecific, endpoints[i].SetIdentifier, "")
				for _, ep := range eps {
