...
func (t *testClient) listRecords(ctx context.Context) ([]*endpoint.Endpoint, error) {
...
func (t *testClient) createRecord(ctx context.Context, ep *endpoint.Endpoint) error {
	t.endpoints = append(t.endpoints, ep)
	t.requests.createRequests = append(t.requests.createRequests, ep)
	return nil
}

func (t *testClient) deleteRecord(ctx context.Context, ep *endpoint.Endpoint) error {
...
func TestNewProvider(t *testing.T) {
	...
	_, err := NewAdGuardHomeProvider(AdGuardHomeConfig{})
	if err == nil {
	...
	_, err = NewAdGuardHomeProvider(AdGuardHomeConfig{Server: srvr.URL})
	if err != nil {
...
func TestProvider(t *testing.T) {
	...
	p := &AdGuardHomeProvider{
		api: &testClient{endpoints: make([]*endpoint.Endpoint, 0), requests: &requests},
	}
