...
// submitRegionChanges implements Regional hostnames functionality for the Change, returns false if it fails
func (p *CloudFlareProvider) submitRegionalHostnameChanges(ctx context.Context, zoneID string, change *cloudFlareChange, resourceContainer *cloudflare.ResourceContainer, logFields log.Fields) bool {
	failedChange := false
	...
	for zoneID, changes := range changesByZone {
		...
		for _, change := range changes {
			...
			if change.Action == cloudFlareUpdate {
				...
				if !p.submitRegionalHostnameChanges(ctx, zoneID, change, resourceContainer, logFields) {
					failedChange = true
				}
