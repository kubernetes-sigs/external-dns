...
func TestCloudFlareProvider_Region(t *testing.T) {
	...
	if provider.RegionalServicesConfig.RegionKey != "us" {
		t.Errorf("expected region key to be 'us', but got '%s'", provider.RegionalServicesConfig.RegionKey)
	}
}
...
func Test_getEndpointRegionalHostname(t *testing.T) {
	type args struct {
		endpoint         *endpoint.Endpoint
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			p := CloudFlareProvider{RegionalServicesConfig: RegionalServicesConfig{Enabled: true, RegionKey: tt.args.defaultRegionKey}}
			got := p.getEndpointRegionalHostname(tt.args.endpoint)
