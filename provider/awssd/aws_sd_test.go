...
func newTestAWSSDProvider(api AWSSDClient, domainFilter provider.DomainFilter, namespaceTypeFilter string) *AWSSDProvider {
...
func TestAWSSDProvider_Records(t *testing.T) {
	...
	provider := newTestAWSSDProvider(api, provider.NewDomainFilter([]string{}), "")
...
func TestAWSSDProvider_ListNamespaces(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"public filter", provider.NewDomainFilter([]string{}), "public", []*sd.NamespaceSummary{namespaceToNamespaceSummary(namespaces["public"])}},
		{"private filter", provider.NewDomainFilter([]string{}), "private", []*sd.NamespaceSummary{namespaceToNamespaceSummary(namespaces["private"])}},
		{"domain filter", provider.NewDomainFilter([]string{"public.com"}), "", []*sd.NamespaceSummary{namespaceToNamespaceSummary(namespaces["public"])}},
		{"non-existing domain", provider.NewDomainFilter([]string{"xxx.com"}), "", []*sd.NamespaceSummary{}},
	} {
