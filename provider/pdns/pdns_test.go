...
var (
	...
	DomainFilterListSingle = provider.DomainFilter{
		Filters: []string{
			"example.com",
		},
	}

	DomainFilterListMultiple = provider.DomainFilter{
		Filters: []string{
			"example.com",
			"mock.com",
		},
	...
	DomainFilterListEmpty = provider.DomainFilter{
		Filters: []string{},
	}
...
func (suite *NewPDNSProviderTestSuite) TestPDNSProviderCreate() {
	...
	_, err = NewPDNSProvider(PDNSConfig{
		Server:       "http://localhost:8081",
		APIKey:       "foo",
		DomainFilter: provider.NewDomainFilter([]string{"example.com", "example.org"}),
	})
