...
func newOCIProvider(client ociDNSClient, domainFilter provider.DomainFilter, zoneIDFilter provider.ZoneIDFilter, dryRun bool) *OCIProvider {
	return &OCIProvider{
		client: client,
		cfg: OCIConfig{
	...
	for name, tc := range testCases {
		t.Run(name, func(t *testing.T) {
			_, err := NewOCIProvider(
				tc.config,
				provider.NewDomainFilter([]string{"com"}),
...
func TestOCIZones(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			domainFilter: provider.NewDomainFilter([]string{"com"}),
			...
			domainFilter: provider.NewDomainFilter([]string{"foo.com"}),
			zoneIDFilter: provider.NewZoneIDFilter([]string{""}),
			expected: map[string]dns.ZoneSummary{
				"foo.com": {
					Id:   common.String("ocid1.dns-zone.oc1..e1e042ef0bfbb5c251b9713fd7bf8959"),
			...
			domainFilter: provider.NewDomainFilter([]string{""}),
