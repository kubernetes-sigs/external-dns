...
func TestAWSZones(t *testing.T) {
	provider := newAWSProvider(t, domains.NewDomainFilter([]string{"ext-dns-test-2.teapot.zalan.do."}), false, []*endpoint.Endpoint{})
...
func TestAWSDeleteRecords(t *testing.T) {
	...
	provider := newAWSProvider(t, domains.NewDomainFilter([]string{"ext-dns-test-2.teapot.zalan.do."}), false, originalEndpoints)
...
func TestAWSApplyChangesDryRun(t *testing.T) {
	...
	provider := newAWSProvider(t, domains.NewDomainFilter([]string{"ext-dns-test-2.teapot.zalan.do."}), true, originalEndpoints)
