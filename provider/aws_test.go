...
func TestAWSSeparateChanges(t *testing.T) {
	...
	changes := separateAWSChanges(zones, changeSet)

	if len(changes) != 2 {
		t.Fatalf("expected %d change(s), got %d", 2, len(changes))
	}

	validateAWSChangeRecords(t, changes["foo-example-org"], []*route53.Change{
		{
			Action: aws.String(route53.ChangeActionCreate),
			ResourceRecordSet: &route53.ResourceRecordSet{
				Name: aws.String("qux.foo.example.org"), TTL: aws.Int64(1),
	...
	validateAWSChangeRecords(t, changes["bar-example-org"], []*route53.Change{
		{
			Action: aws.String(route53.ChangeActionCreate),
			ResourceRecordSet: &route53.ResourceRecordSet{
				Name: aws.String("qux.bar.example.org"), TTL: aws.Int64(2),
	...
	validateAWSChangeRecords(t, changes["baz-example-org"], []*route53.Change{})
}

// func TestAWSCreateRecordsDryRun(t *testing.T) {
// 	provider := newAWSProvider(t, "ext-dns-test-2.teapot.zalan.do.", true, []*endpoint.Endpoint{})
//
// 	records := []*endpoint.Endpoint{
