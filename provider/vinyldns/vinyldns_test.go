...
func testVinylDNSProviderRecords(t *testing.T) {
	mockVinylDNSProvider.domainFilter = provider.NewDomainFilter([]string{"example.com"})
...
func TestNewVinylDNSProvider(t *testing.T) {
	...
	_, err := NewVinylDNSProvider(provider.NewDomainFilter([]string{"example.com"}), provider.NewZoneIDFilter([]string{"0"}), true)
	assert.Nil(t, err)
