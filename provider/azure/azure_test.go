...
func newAzureProvider(domainFilter provider.DomainFilter, zoneIDFilter provider.ZoneIDFilter, dryRun bool, resourceGroup string, zonesClient ZonesClient, recordsClient RecordsClient) *AzureProvider {
...
func TestAzureRecord(t *testing.T) {
	...
	provider := newAzureProvider(provider.NewDomainFilter([]string{"example.com"}), provider.NewZoneIDFilter([]string{""}), true, "k8s", &zonesClient, &recordsClient)
