...
func TestNewWebhookProvider_InvalidURL(t *testing.T) {
	_, err := newProvider("://invalid-url", 5*time.Second, 10*time.Second)
	require.Error(t, err)
}

func TestNewWebhookProvider_HTTPRequestFailure(t *testing.T) {
	_, err := newProvider("http://nonexistent.url", 5*time.Second, 10*time.Second)
	require.Error(t, err)
}
...
func TestNewWebhookProvider_InvalidResponseBody(t *testing.T) {
	...
	_, err := newProvider(svr.URL, 5*time.Second, 10*time.Second)
	require.Error(t, err)
...
func TestValidDomainfilter(t *testing.T) {
	...
	p, err := newProvider(svr.URL, 5*time.Second, 10*time.Second)
...
func TestRecords(t *testing.T) {
	...
	provider, err := newProvider(svr.URL, 5*time.Second, 10*time.Second)
	require.NoError(t, err)
