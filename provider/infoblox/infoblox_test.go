...
func newInfobloxProvider(domainFilter provider.DomainFilter, zoneIDFilter provider.ZoneIDFilter, dryRun bool, client ibclient.IBConnector) *InfobloxProvider {
...
func TestInfobloxRecords(t *testing.T) {
	...
	provider := newInfobloxProvider(provider.NewDomainFilter([]string{"example.com"}), provider.NewZoneIDFilter([]string{""}), true, &client)
