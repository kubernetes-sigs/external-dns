...
From this release, ExternalDNS can become aware of the records it is managing (enabled via `--registry=txt`), therefore ExternalDNS can safely manage non-empty hosted zones. We strongly encourage you to use `v0.3` with `--registry=txt` flag enabled. You might also want to run ExternalDNS in a dry run mode (`--dry-run` flag) to see the changes to be submitted to your DNS Provider API. 
...
```console
$ external-dns --registry txt --provider google --google-project example-project --source service --once --dry-run
```
...
```console
$ external-dns --registry txt --provider google --google-project example-project --source service
```
