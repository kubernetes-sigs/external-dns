...
type WorkloadGroupLister interface {
	...
	List(selector labels.Selector) (ret []*v1.WorkloadGroup, err error)
...
// List lists all WorkloadGroups in the indexer.
func (s *workloadGroupLister) List(selector labels.Selector) (ret []*v1.WorkloadGroup, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.WorkloadGroup))
	})
	return ret, err
}
...
type WorkloadGroupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.WorkloadGroup, error)
...
// List lists all WorkloadGroups in the indexer for a given namespace.
func (s workloadGroupNamespaceLister) List(selector labels.Selector) (ret []*v1.WorkloadGroup, err error) {
...
// Get retrieves the WorkloadGroup from the indexer for a given namespace and name.
func (s workloadGroupNamespaceLister) Get(name string) (*v1.WorkloadGroup, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("workloadgroup"), name)
	}
	return obj.(*v1.WorkloadGroup), nil
}
