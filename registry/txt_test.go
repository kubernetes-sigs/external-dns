...
func testTXTRegistryNew(t *testing.T) {
	...
	_, err := NewTXTRegistry(p, "txt", "", "", time.Hour, "", []string{}, false, nil, "transition")
	require.Error(t, err)

	_, err = NewTXTRegistry(p, "", "txt", "", time.Hour, "", []string{}, false, nil, "transition")
	require.Error(t, err)

	r, err := NewTXTRegistry(p, "txt", "", "owner", time.Hour, "", []string{}, false, nil, "transition")
	...
	_, err = NewTXTRegistry(p, "txt", "txt", "owner", time.Hour, "", []string{}, false, nil, "transition")
	require.Error(t, err)
	...
	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, false, nil, "transition")
	require.NoError(t, err)

	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, false, aesKey, "transition")
	require.NoError(t, err)

	_, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, true, nil, "transition")
	require.Error(t, err)

	r, err = NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, true, aesKey, "transition")
	require.NoError(t, err)
...
func testTXTRegistryRecordsPrefixed(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt.", "", "owner", time.Hour, "wc", []string{}, false, nil, "transition")
...
func testTXTRegistryRecordsSuffixed(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-txt", "owner", time.Hour, "", []string{}, false, nil, "transition")
	records, _ := r.Records(ctx)
...
func testTXTRegistryRecordsNoPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, false, nil, "transition")
	records, _ := r.Records(ctx)
...
func testTXTRegistryApplyChangesWithTemplatedPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "prefix%{record_type}.", "", "owner", time.Hour, "", []string{}, false, nil, "transition")
	changes := &plan.Changes{
		Create: []*endpoint.Endpoint{
			newEndpointWithOwnerResource("new-record-1.test-zone.example.org", "new-loadbalancer-1.lb.com", endpoint.RecordTypeCNAME, "", "ingress/default/my-ingress"),
		},
...
func testTXTRegistryApplyChangesWithTemplatedSuffix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-%{record_type}suffix", "owner", time.Hour, "", []string{}, false, nil, "transition")
	changes := &plan.Changes{
		Create: []*endpoint.Endpoint{
			newEndpointWithOwnerResource("new-record-1.test-zone.example.org", "new-loadbalancer-1.lb.com", endpoint.RecordTypeCNAME, "", "ingress/default/my-ingress"),
		},
...
func testTXTRegistryApplyChangesWithSuffix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "-txt", "owner", time.Hour, "wildcard", []string{}, false, nil, "transition")
...
func testTXTRegistryApplyChangesNoPrefixMetadataOnly(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "", "owner", time.Hour, "", []string{}, false, nil, "metadata-only")
...
func testTXTRegistryMissingRecordsNoPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "", "", "owner", time.Hour, "wc", []string{endpoint.RecordTypeCNAME, endpoint.RecordTypeA, endpoint.RecordTypeNS}, false, nil, "transition")
	records, _ := r.Records(ctx)
...
func testTXTRegistryMissingRecordsWithPrefix(t *testing.T) {
	...
	r, _ := NewTXTRegistry(p, "txt.", "", "owner", time.Hour, "wc", []string{endpoint.RecordTypeCNAME, endpoint.RecordTypeA, endpoint.RecordTypeNS}, false, nil, "transition")
	records, _ := r.Records(ctx)
