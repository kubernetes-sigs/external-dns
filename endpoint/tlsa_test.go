...
func TestNewTLSARecord(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// RFC 6698 section 2.2 permits whitespace inside the certificate
			// association data; the example in section 2.3 is printed this way.
			name:             "whitespace inside the certificate is joined",
			target:           "0 0 1 d2abde240d7cd3ee6b4b28c54df034b9 7983a1d16e8a410e4561cb106618e971",
			wantUsage:        0,
			wantSelector:     0,
			wantMatchingType: 1,
			wantCertificate:  "d2abde240d7cd3ee6b4b28c54df034b97983a1d16e8a410e4561cb106618e971",
		},
		{
			// Cloudflare renders the digest uppercase and in groups.
			name:             "cloudflare grouped rendering is normalised",
			target:           "3 1 1 54EE30E752C6E751B7BFDE666E638EC4 E25F3974207258072299CAA15E15820F",
			wantUsage:        3,
			wantSelector:     1,
			wantMatchingType: 1,
			wantCertificate:  sha256Digest,
...
func TestNewTLSARecordErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{"trailing non-hex junk", "3 1 1 " + sha256Digest + " extra"},
