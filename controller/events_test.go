	...
	}{
		{
			...
			asserts: func(em *fake.EventEmitter, ch plan.Changes) {
				for _, ep := range ch.Create {
					em.AssertCalled(t, "Add", events.NewEvent(ep.RefObject(), ep.EventMsg(), events.ActionCreate, events.RecordReady))
				}
				for _, ep := range ch.Delete {
					em.AssertCalled(t, "Add", events.NewEvent(ep.RefObject(), ep.EventMsg(), events.ActionDelete, events.RecordDeleted))
				}
