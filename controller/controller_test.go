...
func (p *mockProvider) ApplyChanges(changes *plan.Changes) error {
	...
	for i := range changes.Create {
		if changes.Create[i].DNSName != p.ExpectChanges.Create[i].DNSName || !endpoint.TargetSliceEquals(changes.Create[i].Targets, p.ExpectChanges.Create[i].Targets) {
			return errors.New("created record is wrong")
		}
	}

	for i := range changes.UpdateNew {
		if changes.UpdateNew[i].DNSName != p.ExpectChanges.UpdateNew[i].DNSName || !endpoint.TargetSliceEquals(changes.UpdateNew[i].Targets, p.ExpectChanges.UpdateNew[i].Targets) {
			return errors.New("delete record is wrong")
		}
	}

	for i := range changes.UpdateOld {
		if changes.UpdateOld[i].DNSName != p.ExpectChanges.UpdateOld[i].DNSName || !endpoint.TargetSliceEquals(changes.UpdateOld[i].Targets, p.ExpectChanges.UpdateOld[i].Targets) {
			return errors.New("delete record is wrong")
		}
	}

	for i := range changes.Delete {
		if changes.Delete[i].DNSName != p.ExpectChanges.Delete[i].DNSName || !endpoint.TargetSliceEquals(changes.Delete[i].Targets, p.ExpectChanges.Delete[i].Targets) {
			return errors.New("delete record is wrong")
		}
	}
